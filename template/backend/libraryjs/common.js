var module = 'common'
var urlModule = 'http://api.thegioiweb.org/v1.0/common'
	...
	if(param.length){
		param.forEach(function(item, index, array) {
		  	html = html+'<tr style="cursor:pointer;" class="choose">';
				...
				html = html+'<td>';
					html = html+'<input name=order['+item['id']+'] value='+item['order']+' type="text" data-module="service" data-id="'+item['id']+'"  class="form-control sort-order" placeholder="Vị trí" style="width:50px;text-align:right;" >';
				html = html+'</td>';

				html = html+'<td class="client-status" style="text-align:center;">';
					html = html+'<a type="button" href="/user/backend/catalogue/update/'+item['id']+'"   class="btn btn-sm btn-primary btn-update m-r-xs"><i class="fa fa-edit"></i></a>';
