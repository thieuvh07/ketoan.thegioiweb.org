if($('.js_load_view').length){
	check_permission('user/backend/catalogue/view')	
}
if($('.js_load_create').length){
	check_permission('user/backend/catalogue/create')	
}
if($('.js_load_update').length){
	check_permission('user/backend/catalogue/update')	
}

var urlModule = 'http://api.thegioiweb.org/v1.0/user_catalogue'
var fields = 'id,title,created,updated,user_count'
	...
	if(param.length){
		param.forEach(function(item, index, array) {
			html = html+'<tr class="gradeX">';
				...
				html = html+'<td class="text-center">'+item['user_count']+'</td>';
				html = html+'<td class="text-center">'+gettime(item['created'], 'DD-MM-YYYY')+'</td>';
				
				html = html+'<td class="text-center">';
					html = html+'<a type="button" href="user/backend/catalogue/update/'+item['id']+'" class="btn btn-sm btn-primary m-r-xs"><i class="fa fa-edit"></i></a>';
					html = html+'<a type="button" class="btn btn-sm btn-danger ajax_delete" data-id="'+item['id']+'" data-module="user_catalogue" data-moduleText="nhóm thành viên"><i class="fa fa-trash"></i></a>';
				html = html+'</td>';
			html = html+'</tr>';
		});
