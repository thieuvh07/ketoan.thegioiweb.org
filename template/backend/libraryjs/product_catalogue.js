if($('.js_load_view').length){
	check_permission('product/backend/catalogue/view')	
}
if($('.js_load_create').length){
	check_permission('product/backend/catalogue/create')	
}
if($('.js_load_update').length){
	check_permission('product/backend/catalogue/update')	
}

var module = 'product_catalogue'
var urlModule = 'http://api.thegioiweb.org/v1.0/product_catalogue'
var fields = 'id,title,description,user_created,product_count,publish'
	...
	if(param.length){
		param.forEach(function(item, index, array) {
		  	html = html+'<tr style="cursor:pointer;" class="choose">';
				...
				// html = html+'<td class="text-center">'+check_val(item['description'], '-')+'</td>'; 
				html = html+'<td class="text-center">'+item['product_count']+'</td>'; 
				...
				html = html+'<td class="client-status" style="text-align:center;">';
					html = html+'<a type="button" href="/product/backend/catalogue/update/'+item['id']+'"   class="btn btn-sm btn-primary btn-update m-r-xs"><i class="fa fa-edit"></i></a>';
