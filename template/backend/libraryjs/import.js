...
if($('.js_search_product').length){
	...
	$(document).on('change keyup','input[name="product[quantity_import][]"],input[name="product[price][]"]',function(){
		let _this = $(this);
		let money = 0;
		let price = _this.parents('tr').find('input[name="product[price][]"]').val();
		let quantity_import = _this.parents('tr').find('input[name="product[quantity_import][]"]').val();
		price = is0(price);
		quantity_import = is0(quantity_import);

		price = price.replace(/\./gi, "");
		money =  Math.round(mul(quantity_import, price));
...
var module = 'import'
var urlModule = 'http://api.thegioiweb.org/v1.0/import'
var fields = 'id, code, supplierid, pay, date_start,supplier,user_created,total_money,code'
...
function image_render(src = '', name=''){
	let html = '<div class="col-sm-2">';
		...
		html = html + '<input type="text" name="import[title][]" placeholder="Thêm tên sp" value="" autocomplete="off" class="text-center m-b-sm form-control" />';
		html = html + '<input type="text" name="import[code][]" value="'+name+'" class="text-center hidden m-b form-control" />';
		html = html + '<span class="m-b-sm text-center">'+name+'</span><input type="text" name="import[image][]" value="'+src+'" class="hidden" />';
	return html;
}
	...
	if(param.length){
		param.forEach(function(item, index, array) {
		  	html = html+'<tr style="cursor:pointer;" class="choose">';
				...
				// 			html = html+'<input type="checkbox" value="'+item['pay']+'" '+((item['pay'] == 1) ? 'checked=""' : '')+' class="onoffswitch-checkbox" id="pay-'+item['id']+'">'; 
				// 			html = html+'<label class="onoffswitch-label ajax_update_status" data-module="product_catalogue" data-id="'+item['id']+'"  data-field="pay" for="pay-'+item['id']+'">'; 
				...
				html = html+'<td class="client-status" style="text-align:center;">';
					html = html+'<a type="button" href="/import/backend/import/update/'+item['id']+'"   class="btn btn-sm btn-primary btn-update m-r-xs"><i class="fa fa-edit"></i></a>';
...
$(document).ready(function(){
	...
	$(document).on('click','.js_excel', function(){
		let supplierid = $('select[name=supplierid]').val()

		$.ajax({
			type: 'get', 
			url: urlModule+'/excel?supplierid='+supplierid+authid,
			data: '',
			crossDomain:true,
			cache: false,
			success: function(resultApi){
	...
	html= html+ '<tr>';
        ...
        html= html+ ' <td>';
        	html= html+ '<div class="input-group">';
        		html= html+ '<input type="text" autocomplete="off" style="height: 25px" name="product[quantity_import][]" value="1" class="float text-right form-control">';
            html= html+ '</div>';
        html= html+ '</td>';

        html= html+ ' <td>';
        	html= html+ '<div class="input-group">';
        		html= html+ '<input type="text" autocomplete="off" style="height: 25px" name="product[measure_import][]" value="Cuốn" class="text-right form-control">';
            html= html+ '</div>';
        html= html+ '</td>';
...
function total_money_import(){
	...
	$('.js_content tr').each(function (){
		...
		let quantity = _this.find('input[name="product[quantity_import][]"]').val();
