...
if($('.js_load_create').length){
	check_permission('cash/backend/catalogue/create')	
}
if($('.js_load_update').length){
	check_permission('cash/backend/catalogue/update')	
}

var module = 'cash_catalogue'
var urlModule = 'http://api.thegioiweb.org/v1.0/cash_catalogue'
var fields = 'id,title,description,user_created'
	...
	if(param.length){
		param.forEach(function(item, index, array) {
		  	html = html+'<tr style="cursor:pointer;" class="choose">';
				...
				html = html+'<td>'+item['description']+'</td>'; 
				html = html+'<td class="text-center">'+item['user_created']+'</td>'; 
			

				html = html+'<td class="client-status" style="text-align:center;">';
					html = html+'<a type="button" href="/cash/backend/catalogue/update/'+item['id']+'"   class="btn btn-sm btn-primary btn-update m-r-xs"><i class="fa fa-edit"></i></a>';
