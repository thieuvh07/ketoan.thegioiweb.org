...
if($('.js_load_view').length){
	check_permission(module+'/backend/detail/view')	
}

// cập nhập khoản thu chi
$(document).on('click','.js_update',function(){
	let _this = $(this);
	let time = _this.parents('tr').find('.time').html();
	...
	_this.parents('tr').find('.time').html('<input type="text" name="time" value="'+time+'" class="form-control text-center" data-mask="99/99/9999" placeholder="dd/mm/Y" autocomplete="off">');
	...
	$(document).on('click keyup','#construction',function(){
		...
		if(keyword.length > 1){
			setTimeout(function() {
				...
				$.ajax({
					type: 'GET', 
					url: 'http://api.thegioiweb.org/v1.0/construction/index?fields=fullname,code,gross_revenue_real,user_charge,id&query=trash=0,status=0,keyword='+keyword,
					crossDomain:true,
					cache: false,
...
var urlModule = 'http://api.thegioiweb.org/v1.0/cash_detail'
var fields = ''
	...
	if(param.length){
		param.forEach(function(item, index, array) {
		 	html = html+'<tr style="cursor:pointer;" class="choose" data-id="'+item['id']+'">';
				...
				html = html+'</td>';
				html = html+'<td class="text-center time">'+gettime(item['time'], ' DD-MM-YYYY')+'</td>'; 
				...
				if(!check_val(item['extendid']) && (item['catalogueid'] == 1 || item['catalogueid'] == 2 || item['catalogueid'] == 7)){
					bg = 'bg-red'
				}else{
					bg = ''
				}
				html = html+'<td data-id="'+item['extendid']+'" class="extend '+bg+'">'+item['extend']+'</td>'; 
				...
				html = html+'<td class="text-center client-status">';
					...
					html = html+'<a type="button" data-module="cash_detail" data-id="'+item['id']+'" class="btn btn-sm btn-danger ajax_delete"><i class="fa fa-trash"></i></a>';
				html = html+'</td>';
...
$(document).ready(function(){
	...
	$(document).on('click','.js_create', function(){
		...
		let _this =$(this)
		let obj = {}
		obj.time = _this.parents('tr').find('input[name="time"]').val()
